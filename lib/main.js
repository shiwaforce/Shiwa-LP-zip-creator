            ...
            if (path.basename(file).split('.').pop() == 'html'){
                ...
                const promise = new Promise((resolve, reject) => {
                    ...
                    fs.readFile(file, 'utf8', function (err, data) {
                        content = data.replaceAll('="/', `="${path.join("sw","static","landing-pages",path.basename(outputZipFile, '.zip'),"html")}/`);
                ...
                const promise = new Promise((resolve, reject) => {
                    ...
                    let content = ''
                    fs.readFile(file, 'utf8', function (err, data) {
                        //content = data.replaceAll('"static/js/', `"${path.join("sw","static","landing-pages",path.basename(outputZipFile, '.zip'),"html",path.dirname(relativePath))}/`);
                        content = data.replaceAll('="/', `="${path.join("sw","static","landing-pages",path.basename(outputZipFile, '.zip'),"html")}/`);
                        fs.writeFileSync(destinationPath, content);
                        resolve();
                    });
